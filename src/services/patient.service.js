...
class PatientService {
  ...
  async getPatientAppointmentHistory(patientId, options = {}) {
    try {
      ...
      const filter = { "patient.patientId": patientId };

      // Filter by appointment date range
      if (dateRange) {
        if (dateRange.from) {
          filter.date = { $gte: new Date(dateRange.from) };
        }
        if (dateRange.to) {
          filter.date = { ...filter.date, $lte: new Date(dateRange.to) };
        }
      }
      ...
      // Filter by patient registration status if specified
      if (status) {
        appointments = appointments.filter((appointment) => {
          ...
          return (
            patientRegistration &&
            patientRegistration.registrationStatus === status
          );
        });
      }
